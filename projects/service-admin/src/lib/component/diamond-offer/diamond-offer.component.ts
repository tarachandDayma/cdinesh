...
export class DiamondOfferComponent implements OnInit, OnChanges {
  ...
  changeAvgPrice() {
    ...
    this.diamonds.forEach(element => {
      ...
      element._offerPrice = this.avgPrice;
      if (element.rapPrice > 0 && element._offerPrice > 0) {
        element._offerBack = parseFloat((((element._offerPrice / element.rapPrice) - 1) * 100).toFixed(2));
      }
    });
    this.calculateSummary();
  }
  changeAvgBack() {
    this.diamonds.forEach(element => {
      element._offerBack = this.avgBack;
      if (element.rapPrice > 0 && element._offerBack != 0) {
        element._offerPrice = parseFloat((element.rapPrice * (1 - (Math.abs(element._offerBack) / 100))).toFixed(2))
      }
    });
    this.calculateSummary();
  }
  changeOfferPrice(element) {
    if (element.rapPrice > 0 && element._offerPrice > 0) {
      element._offerBack = parseFloat((((element._offerPrice / element.rapPrice) - 1) * 100).toFixed(2));
    }
    this.calculateSummary();
  }
  changeOfferBack(element) {
    if (element.rapPrice > 0 && element._offerBack != 0) {
      element._offerPrice = parseFloat((element.rapPrice * (1 - (Math.abs(element._offerBack) / 100))).toFixed(2))
    }
    this.calculateSummary();
  ...
  saveOffer() {
    ...
    this.diamonds.forEach(element => {
      ...
      offer.offerBack=element._offerBack;
      offer.offerPrice=element._offerPrice;
